...
void GenerateTextureCoords(int frameWidth, int frameHeight, int spriteSheetWidth, int spriteSheetHeight, float textCoordArray[10][8], int frameCount)
{
    ...
    {
        // Вычисляем текстурные координаты для текущего кадра анимации
        ...
        startX += stepX; // Переходим к следующему кадру анимации
    }
}
...
{
    int w, h, cnt;
    unsigned char *data = stbi_load("Static.png", &w, &h, &cnt, 0);
    GenerateTextureCoords(80, 80, w, h, textCoordStatic, frameCount);
    ...
    int w1, h1, cnt1;
    unsigned char *data_walkR = stbi_load("Move.png", &w1, &h1, &cnt1, 0);
    GenerateTextureCoords(80, 80, w1, h1, textCoordWalkRight, frameCount);
    ...
    int w2, h2, cnt2;
    unsigned char *data_walkL = stbi_load("Move.png", &w2, &h2, &cnt2, 0);
    GenerateTextureCoords(80, 80, w2, h2, textCoordWalkLeft, frameCount);
    ...
    int w3, h3, cnt3;
    unsigned char *data_jump = stbi_load("Jump.png", &w3, &h3, &cnt3, 0);
    GenerateTextureCoords(80, 80, w3, h3, textCoordJump, frameCount);
